<div x-data="{ loading: false }" class="relative">
    ...
    <div class="overflow-x-auto">
        <div class="flex flex-wrap gap-4 justify-center p-4 rounded-md">
            ...
            <div>
                <x-input wire:model.live="filters.tdoc" placeholder="T. DOC" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-input wire:model.live="filters.Doc" placeholder="DOC" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-maskable mask="###########" wire:model.live="filters.id_entidades" placeholder="ID ENTIDAD"
                    class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-input wire:model.live="filters.Deski" placeholder="ENTIDAD" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-input wire:model.live="filters.name" placeholder="USUARIO" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-input wire:model.live="filters.id_t04tipmon" placeholder="MONEDA" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
            <div>
                <x-input wire:model.live="filters.estadoMon" placeholder="ESTADO" class="w-full" :disabled="empty($movimientos) && !$hasFiltered" />
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($movimientos as $movimiento)
                        <tr>
                            ...
                            <td
                                class="px-4 py-2 border-b {{ $movimiento->estadoMon == 'URGENTE' ? 'bg-yellow-200' : ($movimiento->estadoMon == 'VENCIDO' ? 'bg-red-200' : ($movimiento->estadoMon == 'PENDIENTE' ? 'bg-green-200' : 'bg-blue-200')) }}">
