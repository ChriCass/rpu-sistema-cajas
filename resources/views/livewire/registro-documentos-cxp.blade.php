<div>
    @if ($visible)
        ...
        <div class="mt-4">
            <x-card title="Registro de Documentos cxp">
                    ...
                    <div class="w-full md:w-6/12 px-2">
                        ...
                        <div class="flex flex-wrap -mx-2 mt-4">
                            ...
                            <div class="w-full md:w-4/12 px-2">
                                <x-input label="Descripción T. Doc:" :disabled="$disableFields"
                                    wire:model.live="tipoDocDescripcion" readonly />
                            </div>
                        ...
                        @if ($destinatarioVisible)
                            <div class="w-full md:w-6/12 px-2">
                                <x-select wire:model.live='nuevoDestinatario' placeholder="selecc." :options="$destinatarios"
                                    option-value="descripcion" option-label="descripcion" label="Destinatario:" />
                            </div>
                        @endif
                    </div>
                    <div class="w-full md:w-3/12 px-2">
                ...
                <div class="flex justify-between mt-4 space-x-2">
                    <div class="flex">
                        ...
                        <div class="flex items-center justify-center">
                            <label for="detraccion" class="text-sm font-medium text-gray-700">Detracción:</label>
                            <x-toggle wire:model.live="detraccion" class="h-4 w-4 text-teal-600" />
                            <div class="w-3/12 ml-5">
                                <x-maskable mask="###" wire:model.live="porcetajeDetraccion" suffix="%" :disabled="!$detraccion" />
                            </div>
                        </div>
                    
                        <!-- Monto de detracción y Monto Neto -->
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center space-x-2">
                                <label for="monto_detraccion" class="text-sm font-medium text-gray-700">Monto de detracción:</label>
                                <x-input wire:model.live="monto_detraccion" id="monto_detraccion" class="w-32" :disabled="!$detraccion" />
                            </div>
                    
                            <div class="flex items-center space-x-2">
                                <label for="monto_neto" class="text-sm font-medium text-gray-700">Monto Neto:</label>
                                <x-input wire:model.live="monto_neto" id="monto_neto" class="w-32" :disabled="!$detraccion" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>  <x-button label="Cancelar" outline secondary /></div>
