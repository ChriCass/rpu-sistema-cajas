    ...
    <div class="bg-teal-600 text-white p-4 rounded-t-lg shadow-md">
        <div class="flex justify-between items-center">
            ...
            <div class="flex-1 text-center">
                <h1 class="text-xl font-bold uppercase">GESTIÓN DE OPERADORES</h1>
            </div>
            <div class="flex items-center">
                <div class="text-white rounded-md px-3 py-2 mr-2">
                    <span class="font-semibold">Registros: {{ $operadores->total() }}</span>
                </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
                <div class="w-full sm:w-1/3">
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="text" wire:model.live.debounce.300ms="search" id="search" class="focus:ring-teal-500 focus:border-teal-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Buscar por nombre...">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($operadores as $operador)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $operador->estado ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $operador->estado ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex">
                                <button wire:click="editar({{ $operador->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </button>
                                <button wire:click="cambiarEstado({{ $operador->id }})" class="{{ $operador->estado ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900' }} mr-3">
                                    @if ($operador->estado)
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <button wire:click="confirmarEliminar({{ $operador->id }})" class="text-red-600 hover:text-red-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" wire:model="nombre" id="nombre" class="mt-1 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" style="text-transform: uppercase">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Está a punto de eliminar este operador. Esta acción no se puede deshacer.
                                    </p>
                                    <p class="text-sm text-red-500 mt-2">
                                        Nota: Si el operador está siendo utilizado en algún parte diario, no podrá ser eliminado.
                                    </p>
                                </div>
                            </div>
                        </div>
