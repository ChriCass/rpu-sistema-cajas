...
class MatrizDeCobrosView extends Component
{
    ...
    public function procesar()
    {
        if ($this->status === 'pendiente') {
            $this->obtenerMatrizDePagosPendientes();
        } elseif ($this->status === 'pagado') {
            $this->obtenerMatrizDePagosPagados();
        } else {
            $this->obtenerMatrizDePagosTodos();
        }
    }
    public function obtenerMatrizDePagosPendientes()
    {
        $this->movimientos = DB::select("
            SELECT 
    id,
    tdoc,
    Doc,
        ...
        FROM (
            ...
            FROM (
                SELECT 
                    id_documentos,
                    id_cuentas,
                    IF(id_cuentas = '1', monto, 0) AS monto,
                    IF(id_cuentas = '2', monto, NULL) AS detraccion,
                    IF(id_cuentas = '1', montodo, 0) AS montodo,
                    IF(id_cuentas = '2', montodo, NULL) AS detracciondo
                FROM (
                    SELECT 
                        id_documentos,
                        id_cuentas,
...
WHERE CONCAT(estadoMon, EstadoDetr) NOT IN ('PAGADOPAGADO', 'PAGADO');
    ...
    public function obtenerMatrizDePagosPagados()
    {
        $this->movimientos = DB::select("
            SELECT 
    id,
...
WHERE CONCAT(estadoMon, EstadoDetr)  IN ('PAGADOPAGADO', 'PAGADO');
    ...
    public function obtenerMatrizDePagosTodos()
    {
        $this->movimientos = DB::select("
            SELECT 
    id,
